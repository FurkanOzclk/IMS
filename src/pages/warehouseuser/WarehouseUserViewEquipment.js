...
import { Container, Box, ListItemText, Card, ListItemIcon, Modal,
    CardContent, ListItem, CardHeader, Stack, Typography, Button,TextField } from '@mui/material';
...
const WarehouseUserViewEquipment = () => {
    ...
    const style = {
      ...
      const updateAssignDate=async ()=>{
        try {
            ...
            const { data } = await axiosInstance.put(`/warehouseuser/assignequipment/${id}`, formData)
            
            assignModalClose();
            toast.error("Assign Date Saved");
        } catch (error) {
    ...
    async function getData() {
        let url = "eq";
        if (type === "eq") {
            url = `warehouseuser/equipment/${id}`
        } else if (type === "type") {
            url = `warehouseuser/equipmenttype/${id}`
        } else if (type === "team") {
            url = `warehouseuser/equipmentteam/${id}`
        }
        try {
    ...
    const updateUnassignDate=async ()=>{
        try {
            ...
            const { data } = await axiosInstance.put(`/warehouseuser/unassignequipment/${id}`, formData)
            unassignModalClose();
        ...
        if (type === "eq") {
            return (
                <Card sx={{ top: +10, width: 1, }}>
                    ...
                    <CardContent>
                        <Cardinfo text={`Team Name: ${type}`} icon={<Groups />} />
...
export default WarehouseUserViewEquipment;