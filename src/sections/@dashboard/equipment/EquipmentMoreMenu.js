...
export default function EquipmentMoreMenu(props) {
  ...
  const deleteEquipment = async () => {
    if (type==="eq") {
      try {
        const { data } = await axiosInstance.delete(`/equipment/${props.id}`);
        setData(data);
      ...
      try {
        const { data } = await axiosInstance.delete(`/equipment/type/${props.id}`);
        setData(data);
        toast.success(data.msg);
      } catch (error) {
        toast.error("Get Data Failed");
      ...
      try {
        const { data } = await axiosInstance.delete(`/equipment/team/${props.id}`);
        setData(data);
        toast.success(data.msg);
      } catch (error) {
        toast.error("Get Data Failed");
  ...
  const editEquipment = () => {
    if (type === "eq") {
      ...
      navigate(`/dashboard/equipments/editequipment?id=${props.id}`, {
        state: {
          id: props.id,
          type: props.type,
          data: props.data,
      ...
      navigate(`/dashboard/equipments/editequipmenttype?id=${props.id}`, {
        // state: {
        //   id: props.id,
        //   type: props.type,
        //   data: props.data
      ...
      navigate(`/dashboard/equipments/editequipmentteam?id=${props.id}`, {
        // state: {
        //   id: props.id,
        //   type: props.type,
        //   data: props.data
  ...
  const viewEq = () => {
    navigate(`/dashboard/equipments/viewequipment?id=${props.id}`, {
      state: {
        id: props.id,
        type: props.type
      }
  ...
  const unassignEquipment=async ()=>{
    try {
      ...
      const { data } = await axiosInstance.post(`/equipment/unassign/${props.id}`,formdata)
