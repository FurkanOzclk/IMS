...
export default function MemberMoreMenu(props) {
    ...
    const deleteMember = async () => {
        ...
        try {
            const { data } = await axiosInstance.delete(`/team/member/${props.id}`);
            setData(data);
    ...
    const editMember = () => {
        navigate(`/dashboard/team/member/editmember?id=${props.id}`, { state: { fullname: props.fullname, email: props.email, phone: props.phone, teamId: props.teamId } })
    }

    const viewMember = () => {
        navigate(`/dashboard/team/member/viewmember?id=${props.id}`)
    } 
